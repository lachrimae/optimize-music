...
class Spectra(list):
    def __init__(self, max_size, spectra=None):
        if not max_size > 0:
            raise ValueError
        ...
        if spectra is not None
            ...
            if l > max_size:
                raise ValueError
            super().__init__(spectra + [None for i in range(max_size - l)])
        else:
            super().__init__([None for i in range(max_size)])
    ...
    def __setitem__(self, index : int, spect : ):
        self._as_samples = None
    ...
    # ratio = 3 would put two samples
    # etc.
    # interpolation_func should implement the interface of log_additive.interpolate(x1, x2, l) =~ l*x1 + (1-l)*x2.
    def interpolate_samples(self, interpolation_ratio : int, interpolation_func):
        ...
        for i in range(len(s) - 1):
            ...
            middle_frames = list()
            for i in range(1, interpolation_ratio):
                t = i / interpolation_ratio
                new_sample = interpolation_func(keyframe1, keyframe2, t)
